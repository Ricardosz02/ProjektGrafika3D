...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (glfwGetKey(window, GLFW_KEY_E) == GLFW_PRESS && !ePressedLastFrame) {
            ...
            float checkX = playerX + cos(playerDir) * interactDist;
            float checkY = playerY + sin(playerDir) * interactDist;
        ...
        if (playerHealth <= 50) {
            ...
            if (playerHealth <= 25) {
                healthAlpha = std::max(healthAlpha, 0.6f + factor * 0.4f);
            }
        }

        if (damageAlpha > 0.0f) {
            damageAlpha -= 1.0f * 0.016f;
            if (damageAlpha < 0.0f) damageAlpha = 0.0f;
        }

        float finalIntensity = std::max(damageAlpha, healthAlpha);
        ...
        if (!gameOver) {
            ...
            for (int x = 0; x < screenWidth; x++) {
                ...
                while (!hit) {
                    ...
                    if (tile == 1 || tile == 9) hit = 1;
                        ...
                        if (d) {
                            if (wX_temp > d->openAmount) {
                                hit = 1;
                                texOffset = d->openAmount;
                            }
                        }
                        else hit = 1;
                    }
                ...
                if (type == 9) blueChannelID = 31.0f;
                    ...
                    Door* d = getDoor(mX, mY);
                    if (d && d->isLocked) blueChannelID = 105.0f;
                    else blueChannelID = 106.0f;
                }
                else if (type == 5) {
                    Door* d = getDoor(mX, mY);
                ...
                for (const auto& decal : wallDecals) {
                    if (decal.x == mX && decal.y == mY && decal.side == side) {
                        float decalWidth = 0.25f;
                        float halfWidth = decalWidth / 2.0f;
                        ...
                        if (tX >= decal.hitX - halfWidth && tX <= decal.hitX + halfWidth) {
                            ...
                            float nS_decal = 1 - 2 * dS_decal / screenHeight;
                            float nE_decal = 1 - 2 * dE_decal / screenHeight;
            ...
            for (const auto& bp : bloodParticles) {
                ...
                s.type = OBJECT_BLOOD;
                s.isWeapon = false;
                s.dist = (playerX - bp.x) * (playerX - bp.x) + (playerY - bp.y) * (playerY - bp.y);
            ...
            if (currentWeapon == 0) {
                ...
                if (isShooting) {
                    ...
                    if (phase < 0.25f) gunID = 32.0f;
                    else if (phase < 0.50f) gunID = 33.0f;
                    else if (phase < 0.75f) gunID = 34.0f;
                    else gunID = 35.0f;

                    if (gunID == 32.0f) glActiveTexture(GL_TEXTURE0 + 29);
                    else if (gunID == 33.0f) glActiveTexture(GL_TEXTURE0 + 30);
                    else if (gunID == 34.0f) glActiveTexture(GL_TEXTURE0 + 31);
                    else if (gunID == 35.0f) glActiveTexture(GL_TEXTURE0 + 32);
                }
                else {
                ...
                float gunID = isShooting ? 27.0f : 10.0f;
