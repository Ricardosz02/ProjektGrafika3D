...
// --- ZMODYFIKOWANY FRAGMENT SHADER Z OBS£UG¥ POD£OGI ---
...
uniform sampler2D floorTexture; // Slot 5
...
void main() {
    if (useTexture) {
        ...
        // --- LOGIKA WYBORU TEKSTURY WG KANA£U NIEBIESKIEGO ---
        if (ourColor.b > 4.9) { 
            // ID 5.0 -> PODLOGA (Floor Casting)
            
            // Obliczamy pozycjê piksela wzglêdem œrodka ekranu (horyzontu)
            float p = (screenHeight / 2.0) - gl_FragCoord.y;
            
            // Unikamy dzielenia przez zero przy horyzoncie
            if (p < 1.0) p = 1.0; 

            float posZ = 0.5 * screenHeight; // Wysokoœæ kamery (po³owa ekranu)
            float rowDistance = posZ / p;

            // Obliczamy wektor promienia dla tego piksela (X)
            float cameraX = (gl_FragCoord.x / screenWidth) * 2.0 - 1.0;
            
            // Wektory kierunku i p³aszczyzny kamery
            float dirX = cos(playerDir);
            float dirY = sin(playerDir);
            float planeX = cos(playerDir + 1.5708); // +90 stopni
            float planeY = sin(playerDir + 1.5708);

            // Rzeczywisty wektor promienia
            float rayDirX = dirX + planeX * cameraX;
            float rayDirY = dirY + planeY * cameraX;

            // Mapujemy na œwiat 2D
            vec2 floorPos = playerPos + rowDistance * vec2(rayDirX, rayDirY);

            // Pobieramy teksturê pod³ogi
            texColor = texture(floorTexture, floorPos);
            
            // Opcjonalne przyciemnienie pod³ogi
        ...
        // Normalizujemy kolor (dla Blue > 1.0)
...
GLuint loadTexture(const char* path) {
    ...
    // Dla pod³ogi wa¿ne jest powtarzanie tekstury (REPEAT)
    ...
    if (data) {
        ...
        std::cout << "Zaladowano teksture " << path << " (W:" << width << ", H:" << height << ")" << std::endl;
...
int main() {
    ...
    window = glfwCreateWindow(screenWidth, screenHeight, "Mini DOOM z Pistoletem i Podloga", NULL, NULL);
    ...
    // Zwiêkszam bufor, aby pomieœci³ dodatkowe wierzcho³ki pod³ogi
    glBufferData(GL_ARRAY_BUFFER, sizeof(float) * screenWidth * 10 * 7 * 2, nullptr, GL_DYNAMIC_DRAW);
    ...
    GLuint floorTextureID = loadTexture("floor.png"); // NOWA TEKSTURA

    GLint useTextureLoc = glGetUniformLocation(shaderProgram, "useTexture");

    glUseProgram(shaderProgram);

    // Przypisanie slotów tekstur
    ...
    glUniform1i(glGetUniformLocation(shaderProgram, "floorTexture"), 5); // Slot 5

    // Pobranie lokalizacji nowych uniformów dla pod³ogi
    ...
    glActiveTexture(GL_TEXTURE5); glBindTexture(GL_TEXTURE_2D, floorTextureID); // Aktywacja pod³ogi
    ...
    std::cout << "Mini DOOM - Start." << std::endl;
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (hasPistol && glfwGetKey(window, GLFW_KEY_SPACE) == GLFW_PRESS && !spacePressedLastFrame) {
            if (ammoCount > 0) {
                ...
                // Prosty Raycast strza³u (na œrodek ekranu)
                float perpWallDist = 1e30f;
                int hitSide = 0;
                int hitMapX = -1, hitMapY = -1;
                float hitTexX = 0.0f;
        ...
        if (!gameOver) {
            ...
            auto checkMove = [&](float dx, float dy) {
                ...
                if (nextMapY >= 0 && nextMapY < MAP_HEIGHT && nextMapX >= 0 && nextMapX < MAP_WIDTH) {
                    ...
                    else if (targetBlock == 9) { // Zmiana mapy
                        if (activeMapIndex == 1) {
        ...
        if (!gameOver) {
            ...
            // Wysy³amy dane do Shadera dla Pod³ogi
            glUniform1f(playerDirLoc, playerDir);
            glUniform2f(playerPosLoc, playerX, playerY);
            glUniform1f(screenWLoc, (float)screenWidth);
            glUniform1f(screenHLoc, (float)screenHeight);

            // --- 1. RYSOWANIE POD£OGI (Blue = 5.0) ---
            float floorR = 1.0f, floorG = 1.0f, floorB = 5.0f;
            // Rysujemy dwa trójk¹ty pokrywaj¹ce doln¹ po³owê ekranu (y od -1 do 0)
            vertices.insert(vertices.end(), { -1.0f, -1.0f, floorR, floorG, floorB, 0.0f, 0.0f });
            vertices.insert(vertices.end(), { 1.0f, -1.0f, floorR, floorG, floorB, 0.0f, 0.0f });
            vertices.insert(vertices.end(), { 1.0f,  0.0f, floorR, floorG, floorB, 0.0f, 0.0f });
            ...
            for (int x = 0; x < screenWidth; x++) {
                ...
                float perpWallDist = (side == 0) ? (mapX - playerX + (1.0f - stepX) / 2.0f) / rayDirX
                    : (mapY - playerY + (1.0f - stepY) / 2.0f) / rayDirY;
                ...
                if (transformY > 0.1f) {
                    ...
                    for (int stripe = drawStartX; stripe < drawEndX; stripe++) {
                        if (stripe >= 0 && stripe < screenWidth && transformY < zBuffer[stripe]) {
                            ...
                            float xposL = 2.0f * stripe / screenWidth - 1.0f;
                            float xposR = 2.0f * (stripe + 1) / screenWidth - 1.0f;
                            vertices.insert(vertices.end(), { xposL, ndcStart, r_flash, g_flash, b_flash, texX, 1.0f, xposR, ndcStart, r_flash, g_flash, b_flash, texX, 1.0f, xposR, ndcEnd, r_flash, g_flash, b_flash, texX, 0.0f });
                            vertices.insert(vertices.end(), { xposL, ndcStart, r_flash, g_flash, b_flash, texX, 1.0f, xposR, ndcEnd, r_flash, g_flash, b_flash, texX, 0.0f, xposL, ndcEnd, r_flash, g_flash, b_flash, texX, 0.0f });
                        }
                    }
                }
            }
            ...
            for (const auto& w : weapons) {
                if (!w.isCollected) {
                    ...
                    Sprite tempSprite; tempSprite.x = w.x; tempSprite.y = w.y; tempSprite.dist = distSq; tempSprite.isWeapon = true;
                ...
                if (transformY > 0.1f) {
                    ...
                    for (int stripe = drawStartX; stripe < drawEndX; stripe++) {
                        if (stripe >= 0 && stripe < screenWidth && transformY < zBuffer[stripe]) {
                            ...
                            float xposL = 2.0f * stripe / screenWidth - 1.0f;
                            float xposR = 2.0f * (stripe + 1) / screenWidth - 1.0f;
                            vertices.insert(vertices.end(), { xposL, ndcStart, r_sprite, g_sprite, b_sprite_final, texX, 1.0f, xposR, ndcStart, r_sprite, g_sprite, b_sprite_final, texX, 1.0f, xposR, ndcEnd, r_sprite, g_sprite, b_sprite_final, texX, 0.0f });
                            vertices.insert(vertices.end(), { xposL, ndcStart, r_sprite, g_sprite, b_sprite_final, texX, 1.0f, xposR, ndcEnd, r_sprite, g_sprite, b_sprite_final, texX, 0.0f, xposL, ndcEnd, r_sprite, g_sprite, b_sprite_final, texX, 0.0f });
                        }
                    }
                }
            }

            // --- 6. HUD (PISTOLET + TEKST) ---
